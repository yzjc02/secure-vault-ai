...
左侧栏分为顶部状态区、中间内容区和底部操作区。顶部状态区显示应用标题、登录状态和语言切换控件。

中间内容区包含 Conversations / 会话 和 Knowledge Files / 知识库文件。会话列表和知识库文件列表都可以独立滚动；会话很多时不会遮挡知识库文件，文档很多时也不会遮挡 Audit Logs 和 Logout。

点击知识库文件区右上角的上传按钮可以上传文档，刷新按钮可以重新加载文档列表。每个文档项上的 Embed / Delete 按钮用于执行向量化或删除操作。

底部操作区固定显示 Audit Logs 和 Logout，方便随时查看 audit logs 或退出登录。
